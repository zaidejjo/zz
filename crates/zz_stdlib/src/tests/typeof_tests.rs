...
fn typeof_builtin() {
    ...
    assert_eq!(
        ...
        Value::Str("dict".to_string().into())
    );
    assert_eq!(
        run("typeof(.some(1))").unwrap(),
        Value::Str("option".to_string().into())
    );
    assert_eq!(
        run("typeof(.ok(1))").unwrap(),
        Value::Str("result".to_string().into())
    );
    assert_eq!(
        run("typeof(0..5)").unwrap(),
        Value::Str("range".to_string().into())
    );
}
...
fn typeof_struct_reports_name() {
    ...
    assert_eq!(v, Value::Str("Point".to_string().into()));
}
