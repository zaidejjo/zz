...
fn known_native_failure(file: &Path) -> Option<&'static str> {
    ...
    match stem {
        ...
        "match" => Some("C codegen: else keyword emitted outside function scope"),
        "return_in_loops" => Some("C codegen: return type mismatch (int64_t vs zz_value)"),
...
parity_known_failure!(parity_syntax_match, "syntax", "match.zz");
